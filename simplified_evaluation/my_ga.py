...
class GA:
    ...
    def GA_main(self):
        ...
        for g in range(NGEN):
            ...
            # self.pop = nextoff
            self.pop += nextoff
            if len(self.pop) >= 1000:
                self.pop = sorted(self.pop, key=itemgetter("fitness"), reverse=True)[:popsize]
                best_ind = self.pop[0]
                if not self.bestindividual or best_ind['fitness'] > self.bestindividual['fitness']:
