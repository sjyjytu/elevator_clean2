...
import numpy as np
import time
from generator_proxy import set_seed
from generator_proxy import PersonGenerator
from mansion_configs import MansionConfig
from mansion_manager import MansionManager
...
class LiftSim:
    ...
    def step(self, hallcall):
        ...
        action_tuple.append(ElevatorHallCall(hallcall[0], hallcall[1]))

        # time_consume, energy_consume, given_up_persons = self._mansion.run_mansion(action_tuple)
        calling_wt, all_people_waiting_time, loaded_person_num, all_enter_person_num, no_io_masks = self._mansion.run_mansion(action_tuple)
