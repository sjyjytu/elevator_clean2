...
class SmecElevator(object):
    ...
    def __init__(self, start_position=0.0, mansion_config=None, name="ELEVATOR", _mansion=None, elev_index=-1):
        self.elev_idx = elev_index
        ...
        self._door_open_velocity = 1.0 / mansion_config.door_opening_time
        self._door_close_velocity = 1.0 / mansion_config.door_closing_time
        ...
        self._door_well_time2 = mansion_config.dwell_time2
        ...
        self._start_floor = 0
        self._delta_floor = 0
        self._door_open_lock = False
        self._door_open_time = 0

        self._mansion = _mansion

        self.sudden_stop = True
        self.arrive_info = []
        self.serving_floor = -1
    ...
    def remove_hall_call(self, hallcall):
        for call in hallcall.HallUpCall:
            if call in self._hall_up_call:
                self._hall_up_call.remove(call)

        for call in hallcall.HallDnCall:
            if call in self._hall_dn_call:
                self._hall_dn_call.remove(call)

    def replace_hall_call(self, hallcall):
        self._hall_up_call = [i for i in hallcall.HallUpCall]
        self._hall_dn_call = [i for i in hallcall.HallDnCall]

    def set_park_call(self, parkcall):
        self._park_call = parkcall

    def remove_cur_park_call(self):
        self._park_call = -1

    def is_park_call_arrive(self, advance_floor):
        return self._park_call != -1 and advance_floor == self._park_call and self._loaded_person[self._park_call] == []

    def acquire_hall_serving(self):
        if self._mansion.not_in_hallcall_but_serving_by[self._sync_floor] == -1:
            self._mansion.not_in_hallcall_but_serving_by[self._sync_floor] = self.elev_idx
            self.serving_floor = self._sync_floor

    def release_hall_serving(self):
        if self._mansion.not_in_hallcall_but_serving_by[self._sync_floor] == self.elev_idx:
            self._mansion.not_in_hallcall_but_serving_by[self._sync_floor] = -1
            self.serving_floor = -1

    def remove_cur_floor_hall_call(self, time_cnt=None):
        if (self._service_direction == 1) and self._sync_floor in self._hall_up_call:
            self._hall_up_call.remove(self._sync_floor)
            self._mansion.updn_last_serve_time[self._sync_floor] = self._config.raw_time

            # self.acquire_hall_serving()
            if time_cnt:
                self.arrive_info.append(('u' + str(self._sync_floor),
            ...
            self._mansion.updn_last_serve_time[self._sync_floor+self._number_of_floors] = self._config.raw_time

            # self.acquire_hall_serving()
            if time_cnt:
                self.arrive_info.append(('d' + str(self._sync_floor),
                                         time_cnt.get_time(self._config._delta_t)))

    def remove_cur_floor_car_call(self, time_cnt=None):
        if self._sync_floor in self._car_call:
            self._car_call.remove(self._sync_floor)
            # self._mansion.car_last_finish_time[self._sync_floor] = self._config.raw_time
            if time_cnt:
    ...
    def run_elevator(self, with_bug=False, time_cnt=None):
        ...
        tmp_velocity, eff_dt = velocity_planner(self._current_velocity, remain_distance, acc,
        ...
        if abs(tmp_velocity) > EPSILON:
            ...
            delta = 0.0
            # refine the position caused by the dt.
        ...
        if self._run_state == ELEVATOR_STOP_DOOR_CLOSE:
            ...
            #         and len(self._loaded_person[self._sync_floor]) == 0:  # add by JY, wait all person get off the car.
                ...
                # self.release_hall_serving()
                self._target_floor = target_floor
                self._start_floor = current_floor
                self._delta_floor = abs(self._target_floor - self._start_floor)
                ...
                if self.is_park_call_arrive(advance_floor):
                    self.remove_cur_park_call()
                else:
                    self._run_state = ELEVATOR_STOP_DOOR_OPENING
                    self.remove_cur_floor_car_call()
            self._current_position = self.get_floor_position(self._sync_floor)
        elif self._run_state == ELEVATOR_RUN:
            if target_floor != GO_FLOOR_NONE:
                self._target_floor = target_floor
            if abs(self._current_velocity) < EPSILON and (target_floor == advance_floor): # modified by Zelin
                if self.is_park_call_arrive(advance_floor):
                    self.remove_cur_park_call()
                    self._run_state = ELEVATOR_STOP_DOOR_CLOSE
                    self._current_position = self.get_floor_position(self._sync_floor)
                    ...
                    self._run_state = ELEVATOR_STOP_DOOR_OPENING
                    self.remove_cur_floor_car_call()
            elif target_floor == GO_FLOOR_NONE:
            ...
            if self._door_open_rate > 1.0 - EPSILON:
                ...
                # self.remove_cur_floor_car_call(time_cnt)
            ...
            if self._door_open_rate < EPSILON:
                self._run_state = ELEVATOR_STOP_DOOR_CLOSE
                self.remove_cur_floor_hall_call()
        else:
            pass
        ...
        # if run this, there will be a bug.
        # if with_bug:
        #     if self.is_fully_open and self.is_stopped:
        #         self.remove_cur_floor_hall_call(time_cnt)
        # else:
        #     # suppose that when the door is closing, the coming person should choose another car.
        #     if self._is_door_closing and self._door_open_rate < EPSILON:
        #         self.remove_cur_floor_hall_call(time_cnt)
        ...
        if self._is_door_opening:
            self._door_open_rate = min(
                ...
                1.0, self._door_open_rate + self._door_open_velocity * self._dt)  # debug by JY
        elif self._is_door_closing:
            self._door_open_rate = max(
                0.0, self._door_open_rate - self._door_close_velocity * self._dt)
        ...
        if len(self._entering_person) != 0:
            ...
            if self._entering_person[0][1] < EPSILON:
                ...
                self._mansion.elevators_car_call_change = True

        # add passengers to exiting queue
        if self.is_stopped and self.is_fully_open:
            # if abs(delta_distance) < EPSILON:
            if not self._door_open_lock:
                self._door_open_lock = True
                self._door_open_time = self._config.raw_time
            ...
            if len(self._loaded_person[floor_idx]) > 0:
                ...
                if self._mansion:
                    att = self._door_open_time - self._mansion.person_info[tmp_unload_person.ID][3]
                    self._mansion.person_info[tmp_unload_person.ID].append(att)  # add by JY
                    self._mansion.finish_person_num += 1
                # print(att)
    ...
    @property
    def is_idle_stop(self):
        """
        Returns:
          whether the elevator stops
        """
        return (abs(self._current_velocity) < EPSILON) and (self._hall_up_call == [] and self._park_call == -1) \
               and (self._hall_dn_call == []) and (self._car_call == [] and self._run_state == ELEVATOR_STOP_DOOR_CLOSE)
