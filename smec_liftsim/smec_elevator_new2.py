...
class SmecElevator(object):
    ...
    def get_next_stop_floor(self):
        ...
        if abs(cur_vel) < EPSILON:
            return int(cur_pos // self._floor_height)
        ...
        if self._run_state == ELEVATOR_STOP_DOOR_CLOSE:
            if target_floor != GO_FLOOR_NONE and target_floor != advance_floor:
                ...
                if current_floor < self._target_floor:
                    ...
                    self._run_direction = -1
            elif target_floor != GO_FLOOR_NONE and target_floor == advance_floor and abs(
                    self._current_velocity) < EPSILON:
