...
class SmecRolloutStorage(object):
    def __init__(self, num_steps, env_num, elevator_num, floor_num):
        ...
        self.rewards = torch.zeros(num_steps, env_num)
    ...
    def reset(self):
        self.rewards = torch.zeros(self.num_steps, self.env_num)
    ...
    def insert(self, obs, actions, action_log_probs, value_preds, rewards, masks):
        ...
        self.rewards[self.step].copy_(torch.tensor(rewards).squeeze())
        ...
        if use_proper_time_limits:
            ...
            if use_gae:
                ...
                for step in reversed(range(self.rewards.shape[0])):
                    td_target = self.rewards[step].unsqueeze(1) + gamma * self.value_preds[step + 1] * self.masks[step + 1].unsqueeze(-1)
