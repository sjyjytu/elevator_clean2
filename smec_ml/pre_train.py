...
model = PretrainModel3(16, 3)
# model.load_state_dict(torch.load('./save_models/model_0026.pt'))
...
def train():
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch + 1) % 1 == 0:
            ...
            if loss < best_loss:
                best_loss = loss
                torch.save(model.state_dict(), f'./save_models3/model_{str(epoch + 1).zfill(4)}.pt')
                print(f'Saving model {str(epoch + 1).zfill(4)}.pt with test loss {loss:.6f} ...')
