...
def generate_person_init(flow_map,init_floor):
    ret_persons = []
    # 按概率随机生成人，根据电梯速度和方向，指定方向和位置来生成？不生成第三趟的人
    id = 0
    for i in range(5):
        ...
        for src in range(samples.shape[0]):
            for dst in range(samples.shape[1]):
                if(dst>init_floor)and(src<init_floor)and(samples[src, dst] < flow_map[src, dst]):
                    ret_persons.append(
                        PersonType(
                            id,
                            75,
...
if __name__ == '__main__':
    ...
    for d in range(50000):
        ...
        init_persons=generate_person_init(flow_map,init_floor)
