...
class SmecRolloutStorage(object):
    def __init__(self, num_steps, env_num, elevator_num, floor_num):
        self.num_steps, self.env_num, self.elev_num, self.floor_num = num_steps, env_num, elevator_num, floor_num
        self.rewards = torch.zeros(num_steps, env_num, elevator_num)
        self.value_preds = torch.zeros(num_steps + 1, env_num, elevator_num)
        self.returns = torch.zeros(num_steps + 1, env_num, elevator_num)
        self.action_log_probs = torch.zeros(num_steps, env_num, elevator_num)
        self.actions = torch.zeros(num_steps, env_num, elevator_num)
        self.actions = self.actions.long()
        self.masks = torch.ones(num_steps + 1, env_num)
        self.step = 0
        zero_obs = OrderedDict({'upload_wait_nums': torch.zeros(env_num, floor_num, ),
                                ...
                                'legal_masks': torch.zeros(env_num, floor_num * 2, )})
        self.obs = [zero_obs.copy() for j in range(num_steps + 1)]
        self.reset()

    def reset(self):
        self.rewards = torch.zeros(self.num_steps, self.env_num, self.elev_num)
        self.value_preds = torch.zeros(self.num_steps + 1, self.env_num, self.elev_num)
        self.returns = torch.zeros(self.num_steps + 1, self.env_num, self.elev_num)
        self.action_log_probs = torch.zeros(self.num_steps, self.env_num, self.elev_num)
        self.actions = torch.zeros(self.num_steps, self.env_num, self.elev_num)
        self.actions = self.actions.long()
        self.masks = torch.ones(self.num_steps + 1, self.env_num)
        self.step = 0
        zero_obs = OrderedDict({'upload_wait_nums': torch.zeros(self.env_num, self.floor_num, ),
                                'download_wait_nums': torch.zeros(self.env_num, self.floor_num, ),
                                'elevator_loading_maps': torch.zeros((self.env_num, self.elev_num, self.floor_num)),
                                'elevator_location_maps': torch.zeros((self.env_num, self.elev_num,)).long(),
                                'legal_masks': torch.zeros(self.env_num, self.floor_num * 2, )})
        self.obs = [zero_obs.copy() for j in range(self.num_steps + 1)]
        ...
        if use_proper_time_limits:
            ...
            if use_gae:
                ...
                for step in reversed(range(self.rewards.shape[0])):
                    td_target = self.rewards[step] + gamma * self.value_preds[step + 1] * self.masks[step + 1].unsqueeze(-1)
